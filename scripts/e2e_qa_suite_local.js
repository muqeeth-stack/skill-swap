...
async function runQA() {
  ...
  async function waitEl(sel, ms = 15000) {
  ...
  try {
    ...
    await testJourney(4, '5-Step Registration Wizard', async () => {
      ...
      await Promise.race([
        page.waitForNavigation({ waitUntil: 'networkidle0', timeout: 18000 }).catch(() => {}),
        page.waitForFunction(() => window.location.pathname.includes('/matches'), { timeout: 18000 })
      ]);
    ...
    await testJourney(26, 'Password Login, Logout & Remember-Me', async () => {
      ...
      await Promise.race([
        page.waitForNavigation({ waitUntil: 'networkidle0', timeout: 18000 }).catch(() => {}),
        page.waitForFunction(() => window.location.pathname.includes('/dashboard'), { timeout: 18000 })
      ]);
